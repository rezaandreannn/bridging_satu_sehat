...
<?php
  ...
  <script src="<?php echo base_url(); ?>assets/modules/chart.min.js"></script>
<?php
} elseif ($this->uri->segment(2) == "antrean") { ?>
  ...
  <script src="<?php echo base_url(); ?>assets/modules/select2/dist/js/select2.full.min.js"></script>
<?php
} elseif ($this->uri->segment(2) == "pendaftaran") { ?>
  <script src="<?php echo base_url(); ?>assets/modules/datatables/datatables.min.js"></script>
  <script src="<?php echo base_url(); ?>assets/modules/datatables/DataTables-1.10.16/js/dataTables.bootstrap4.min.js"></script>
  <script src="<?php echo base_url(); ?>assets/modules/datatables/Select-1.2.4/js/dataTables.select.min.js"></script>
  <script src="<?php echo base_url(); ?>assets/modules/jquery-ui/jquery-ui.min.js"></script>
  ...
  <script src="<?php echo base_url(); ?>assets/modules/jquery-selectric/jquery.selectric.min.js"></script>
<?php
} elseif ($this->uri->segment(2) == "dokter") { ?>
  <script src="<?php echo base_url(); ?>assets/modules/datatables/datatables.min.js"></script>
  <script src="<?php echo base_url(); ?>assets/modules/datatables/DataTables-1.10.16/js/dataTables.bootstrap4.min.js"></script>
  <script src="<?php echo base_url(); ?>assets/modules/datatables/Select-1.2.4/js/dataTables.select.min.js"></script>
  <script src="<?php echo base_url(); ?>assets/modules/jquery-ui/jquery-ui.min.js"></script>
...
<?php
  ...
  <script>
    function resetForm() {
      $('#kode_dokter').val(null).trigger('change');
      document.getElementById("tanggal").value = "";
      $('#status_rawat').val(null).trigger('change');
      alert('Filter telah direset!');
      window.location.href = "<?php echo base_url('kunjungan/pendaftaran'); ?>";
    }
  </script>
<?php
} elseif ($this->uri->segment(2) == "antrean") { ?>
  ...
  <script>
    function resetForm() {
      ...
      document.getElementById("tanggal").value = "";
      alert('Filter telah direset!');
      window.location.href = "<?php echo base_url('kunjungan/antrean'); ?>";
    }
  </script>
<?php
} elseif ($this->uri->segment(2) == "dokter") { ?>
...
<?php
  ...
  <script>
    function resetForm() {
      ...
      alert('Filter telah direset!');
      window.location.href = "<?php echo base_url('masterdata/pasien'); ?>";
    }
  </script>
  <script>
    ...
    $(document).ready(function() {
      ...
      $('.btnEdit').click(function() {
        var id = $(this).data('id');
        $.ajax({
          url: '<?php echo base_url("master-data/pasien/edit/") ?>' + id,
          type: "GET",
          dataType: 'json',
          success: function(res) {
            ...
            $('#myModal').modal('show');
            $('#id').val(res.id);
            $('#tanggal_register').val(res.data.pasien.tanggal_register);
            $('#no_mr').val(res.data.pasien.no_mr);
            $('#nama_pasien').val(res.data.pasien.nama_pasien);
            $('#nik').val(res.data.pasien.nik);
            $('#no_hp').val(res.data.pasien.no_hp);
            $('#tanggal_lahir').val(res.data.pasien.tanggal_lahir);
            $('#jenis_kelamin').val(res.data.pasien.jenis_kelamin);
            $('#agama').val(res.data.pasien.agama);
            $('#warga_negara').val(res.data.pasien.warga_negara);
            $('#pekerjaan').val(res.data.pasien.pekerjaan);
            $('#status_kawin').val(res.data.pasien.status_kawin);
            $('#alamat').val(res.data.pasien.alamat);
            $('#provinsi').val(res.data.pasien.provinsi);
            $('#kota').val(res.data.pasien.kota);
            $('#kode_pos').val(res.data.pasien.kode_pos);
            // Populate other fields similarly
          },
          error: function(xhr, status, error) {
            // Handle error
            console.error(xhr.responseText); // Tampilkan respon pesan kesalahan di konsol browser
            alert('Error: ' + xhr.responseText); // Tampilkan pesan kesalahan kepada pengguna
          }
        });
      });
    });
